...
ds_train = load_dataset(DATASET_NAME, split="test").shuffle(seed=42)
...
n_sym = min(TARGET_PER_CLASS, (matches["sym_flag"] == 1).sum())
n_asym = min(TARGET_PER_CLASS, (matches["sym_flag"] == 0).sum())
...
out_df["target"] = out_df["symmetry"].map({
    "symmetric": "entailment",
    "asymmetric": "neutral"
})

out_path = Path("/home/nfarnsworth/NLP-Final-Project/data/sym_test_iid.tsv")
...
plt.title("Distribution of Relations in Sampled Test Data", fontsize=14, weight="bold")
plt.gca().invert_yaxis()  
plt.tight_layout()

chart_path = Path("/home/nfarnsworth/NLP-Final-Project/data/symmetry_test_relation_distribution.png")
